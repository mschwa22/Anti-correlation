...
for clust in range(len(final_clusters)):
    ...
    for i in final_clusters[clust]:
        ...
        plt.errorbar((UV_DIBs.iloc[0]/UV_DIBs.iloc[1]).transform(np.log, axis = 0), (spec_DIBs.iloc[num].div(max_EW)), fmt = 'o')
...
# Create plots of the mean EW of 3 yellow DIBs with the DIBs 5780, 5797, 5418
test_DIBs = [5780, 5797, 5418]
for i in test_DIBs:
    test_num = wavelengths.index(i)
    test_DIB = spec_DIBs.iloc[test_num].mul(sight_data.iloc[0])
    for k in group_4:
        num = wavelengths.index(k)
        yell_DIB_raw = spec_DIBs.iloc[num].mul(sight_data.iloc[0])
        plt.scatter(test_DIB, yell_DIB_raw)
    
    plt.xlabel('{0} EW'.format(i))
    plt.ylabel('EW')
    plt.title('EW of Yellow DIBs With Known DIB')
    plt.show()

# Make a correlation matrix of the 3 yellow DIBs, normalized, and the EB-V and 5780
yellow_DIBs = pd.DataFrame(columns = spec_DIBs.columns)
for i in group_4:
    num = wavelengths.index(i)
    norm_data = spec_DIBs.iloc[num]
    yellow_DIBs = yellow_DIBs.append(norm_data, ignore_index = True)

new_ind = group_4
# Add 5780 to the correlation matrix
num = wavelengths.index(5780)
norm_data = spec_DIBs.iloc[num]
yellow_DIBs = yellow_DIBs.append(norm_data, ignore_index = True)
new_ind.append(5780)

yellow_DIBs.index = new_ind
...
fig, ax = plt.subplots(figsize = (15,10))
sb.heatmap(pears_corr_yellow, xticklabels = yellow_DIBs.index, yticklabels = yellow_DIBs.index, cmap='viridis', ax = ax, annot = True, linewidth = 0.5).set(title = 'Correlation Between Yellow DIBs EW and EB-V Normalized')
plt.show()


# Make a correlation matrix between the 3 yellow DIBs EW (not normalized) and the EB-V and also 5780
yellow_DIBs.iloc[0:4] = yellow_DIBs.iloc[0:4].mul(sight_data.iloc[0])
pears_corr_yellow = yellow_DIBs.T.corr(method = 'pearson')
# Create a heatmap so that the correlations can easily be visualized
fig, ax = plt.subplots(figsize = (15,10))
sb.heatmap(pears_corr_yellow, xticklabels = yellow_DIBs.index, yticklabels = yellow_DIBs.index, cmap='viridis', ax = ax, annot = True, linewidth = 0.5).set(title = 'Correlation Between Yellow DIBs EW and EB-V (Raw Data)')
plt.show()
